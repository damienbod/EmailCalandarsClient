...
Imports System.IO
Imports VbEmailClient.EmailCalendarsClient.MailSender

Partial Public Class MainWindow
    Inherits Window

    Private _aadGraphApiDelegatedClient As AadGraphApiDelegatedClient = New AadGraphApiDelegatedClient()
    Private _emailService As EmailService = New EmailService()
    ...
    Private Async Sub SendEmail(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Dim message = _emailService.CreateStandardEmail(EmailRecipientText.Text, EmailHeader.Text, EmailBody.Text)
        Await _aadGraphApiDelegatedClient.SendEmailAsync(message)
        _emailService.ClearAttachments()
    End Sub

    Private Async Sub SendHtmlEmail(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Dim messageHtml = _emailService.CreateHtmlEmail(EmailRecipientText.Text, EmailHeader.Text, EmailBody.Text)
        Await _aadGraphApiDelegatedClient.SendEmailAsync(messageHtml)
        _emailService.ClearAttachments()
    End Sub

    Private Sub AddAttachment(ByVal sender As Object, ByVal e As RoutedEventArgs)
        Dim dlg = New OpenFileDialog()

        If dlg.ShowDialog() = True Then
            Dim data As Byte() = File.ReadAllBytes(dlg.FileName)
