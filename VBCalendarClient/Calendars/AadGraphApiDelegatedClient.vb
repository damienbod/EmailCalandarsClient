...
Namespace Calendars
    Public Class AadGraphApiDelegatedClient
        ...
        Public Async Function GetCalanderForUser(ByVal email As String, ByVal from As String, ByVal [to] As String) As Task(Of IUserCalendarViewCollectionPage)
            Dim result = Await AcquireTokenSilent()
            ...
            Dim queryOptions = New List(Of QueryOption)() From {
                New QueryOption("startDateTime", from),
                New QueryOption("endDateTime", [to])
            }
            Dim calendarView = Await graphClient.Users(upn).CalendarView.Request(queryOptions).[Select]("start,end,subject,location,sensitivity, showAs, isAllDay").GetAsync()
            Return calendarView
        End Function
